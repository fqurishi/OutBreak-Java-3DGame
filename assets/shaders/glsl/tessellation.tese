...
{	// tc is the color coordinate. This is what enables the tiling effect with GL_REPEAT or MIRRORED_REPEAT
	...
	vec2 th = vec2( (tcs_out[0].x + (gl_TessCoord.x) / patchSize) * tileHx, (tcs_out[0].y + (1.0-gl_TessCoord.y) / patchSize) * tileHy);
	...
	float colorHeight = texture(tex_height, th).r / 64.0;
